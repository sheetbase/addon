import {getActiveFolder, getFolderByName, getFileByName} from './drive.service';
import {getProperty, getProperties, setProperties} from './properties.service';

import {ProjectBuiltinSettings, ProjectSettings} from '../types/addon.type';
...
export function getProjectFolderWithInfo(
...
export function getSettings(freshBuiltin = false) {
  ...
  if (!!freshBuiltin || !settings[PROJECT_ID_KEY]) {
    ...
    const database = getDatabaseFile(folder) as GoogleAppsScript.Drive.File;
    const backend = getBackendFile(folder) as GoogleAppsScript.Drive.File;
    ...
    } as ProjectBuiltinSettings) as Record<string, string>);
...
export function setSettings(settings: ProjectSettings) {
  return setProperties(settings as Record<string, string>);
}
