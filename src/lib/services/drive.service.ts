...
export function isDriveFileId(id: string) {
  ...
  return id.substr(0, 1) === '1' && id.length > 30 && id.length < 35;
}
...
export function buildDriveFileUrl(id: string, type: 'view' | 'edit' | 'uc') {
  return (
    DRIVE_URL + '/' + (type === 'uc' ? `uc?id=${id}` : `file/d/${id}/${type}`)
  );
}
...
export function extractDriveFileId(url: string) {
  ...
  const str1 = url
    ...
    .split('/')
    .shift() as string;
  const str2 = str1.split('id=').pop() as string;
  const str3 = str2.split('&').shift() as string;
  return str3;
}
...
export function createFile(
  ...
  const share =
    typeof sharing === 'string' ? DRIVE_SHARING_PRESETS[sharing] : sharing;
  // create file
  return parentFolder
    .createFile(blob)
    .setSharing(
      DriveApp.Access[
        ((share.access as unknown) as number) || 'PRIVATE'
      ] as GoogleAppsScript.Drive.Access,
      DriveApp.Permission[
        ((share.permission as unknown) as number) || 'NONE'
      ] as GoogleAppsScript.Drive.Permission
    );
}
...
export function createFileFromBase64(
  ...
  return createFileFromBase64Body(
    parentFolder,
    name,
    mimeType,
    base64Body,
